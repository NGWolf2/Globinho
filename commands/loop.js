...
module.exports = {
  name: "loop",
  description: "Liga ou desliga o looping da música.",
  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        .setColor('#fc4e03')
        .setAuthor({
        name: 'Deixe sua música favorita te dominar........... foi mal, toh sem criatividade',
        ...
        .setDescription('**Escute a sua música até você não aguentar mais!**')
      ...
      interaction?.reply({ embeds: [embed], components: [button], fetchReply: true }).then(async Message => {
        ...
        col.on('collect', async (button) => {
          ...
          if (!queue1 || !queue1.playing) {
            await interaction?.editReply({ content: '⚠️ Não tem nenhuma música tocando!!', ephemeral: true }).catch(e => { })
            await button?.deferUpdate().catch(e => {})
          }
          switch (button.customId) {
            case 'queue':
              const success = queue.setRepeatMode(2);
              interaction?.editReply({ content: `✅ Dando looping na fila!!` }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
            case 'nowplaying':
              const success2 = queue.setRepeatMode(1);
              interaction?.editReply({ content: `✅ Loop ativado!!` }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
            case 'close':
              if (queue.repeatMode === 0) {
                await button?.deferUpdate().catch(e => {})
                return interaction?.editReply({ content: '⚠️ O loop já esta desligado!!', ephemeral: true }).catch(e => { })
              }
              const success4 = queue.setRepeatMode(0);
              interaction?.editReply({ content: '▶️ Loop desligado' }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
          }
        })
