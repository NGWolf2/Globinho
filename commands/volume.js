...
module.exports = {
  name: "volume",
  description: "Altere o volume da música.",
  permissions: "0x0000000000000800",
  options: [{
    name: 'volume',
    description: 'Digite a porcentagem da música.',
  ...
  run: async (client, interaction) => {
    try {
      ...
      if (queue.volume === vol) {
        return interaction.reply({ content: 'O volume atual já é **' + vol + '**!', ephemeral: true });
      }

      if (vol < 1 || vol > maxVol) {
        return interaction.reply({
          content: `Digite um numero entre \`1\` e \`${maxVol}\`.`,
          ephemeral: true
        });
      }
      ...
      if (success) {
        ...
        return interaction.reply({ content: '❌ Algo errado aconteceu mudando o volume.', ephemeral: true });
      }
    } catch (e) {
      console.error(e);
    }
